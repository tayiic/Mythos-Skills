...
## 十、v002 方法论扩展：从 Skill 编写到软件工程全生命周期

### 10.1 方法论缺口分析

当前 M1-M6 覆盖了"如何写好 Skill"，但对比 superpowers 的完整工作流，我们缺少**编码前和编码后**的方法论：

```
superpowers 完整链路:
  [brainstorming] → [writing-plans] → [executing-plans] → [TDD] → [review] → [verify]
...
| 方法论 | 名称 | 核心思想 | 状态 |
|--------|------|---------|------|
| **M7** | 计划驱动开发 (Plan-Driven Development) | 不是教 AI 写计划，而是定义什么是一个好计划的标准。结合 P1-P8：计划本身也是"上下文"，需要简洁、渐进式披露 | 🔴 待创建 |
| **M8** | 代码审查框架 (Code Review Framework) | 显式化审查标准：审什么（正确性+安全性+可测试性）、用什么标准（P1-P8 同样适用审查）、产出是什么（APPROVED / REQUEST_CHANGES） | 🔴 待创建 |
| **M9** | 验证三角 (Verification Triad) | 防止"我觉得修好了"综合征：Unit Test Pass → Integration Smoke → Production Sanity Check | 🟡 待提取 |

### 10.3 完整方法论矩阵 (M1-M9)

| 类别 | 方法论 | 状态 |
|------|--------|------|
| Skill 编写 | M1 P1-P8 原则 | ✅ 已有 |
| Skill 编写 | M2 三层架构 | ✅ 已有 |
| Skill 编写 | M6 创建引擎 | ✅ 已有 |
| 软件工程 | **M7 计划驱动开发** | 🔴 待创建 |
| 软件工程 | **M8 代码审查框架** | 🔴 待创建 |
...
### 10.4 首发 8 Skills 覆盖完整开发生命周期

| # | Skill 名称 | 对应方法论 | 新建/已有 | 优先级 |
|---|-----------|-----------|----------|--------|
| S1 | **plan-forge** | M7 计划驱动 | 🔴 新建 | P0 |
| S2 | implement-standard | M1+M6 | ✅ 已有 | P0 |
| S3 | root-cause-fix | M1+M6 | ✅ 已有 | P0 |
| S4 | **code-review-gate** | M8 审查框架 | 🔴 新建 | P0 |
| S5 | **verify-tri-jump** | M9 验证三角 | 🟡 提取 | P0 |
| S6 | **brainstorm-clarify** | M7 (前置) | 🔴 新建 | P1 |
| S7 | **subagent-orchestrator** | M11 子代理编排 | 🔴 新建 | P1 |
| S8 | **security-hardening** | M3+M8 | 🔴 新建 | P1 |
...
审查现有 `trae_standard_harness` 中的 Skill 实现，发现以下系统性问题需要在 Mythos-Skills 中解决：

| 问题 | 现有 Skill 表现 | P1-P8 违规 | Mythos-Skills 改进方向 |
|------|----------------|-----------|---------------------|
| 跨目录依赖 | 所有 Skill 引用 `../GLOBAL_SKILL_PROTOCOL.md` | **P3 自包含** | 每个 Skill 自包含，零外部引用 |
| SKILL.md 膨胀 | orchestrator 45行+但含大量隐式规则 | **P1 简洁** | SKILL.md ≤55行，详细内容放 references/ |
| 触发描述模糊 | `description` 写"做什么"而非"何时用" | **P5 触发优先** | description 改为触发条件 |
| 无反模式 | Skill 只说正确做法，不列常见错误 | **P6 踩坑即金** | 每个 Skill 必须包含 gotchas |
| 状态存上下文 | FSM 阶段、Token 等存在对话上下文中 | **P8 记忆外置** | 状态写入文件，不依赖对话记忆 |
| 项目耦合 | 依赖 `harness_guard.py`、特定路径 | **P3 自包含** | 零项目特定依赖，纯文本 Skill |
...
| v0.2 | 2026-05-06 | 新增第十节：v002 方法论扩展 (M7-M9)、8 Skills 生命周期计划、已有项目优化洞察 |
